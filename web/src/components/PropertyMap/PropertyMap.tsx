...
const getMarkerConfig = (isRecommended: boolean, zoom: number) => {
  if (isRecommended) {
    return {
      size: 52,
      isSolid: false,
...
const createIcon = (
  ...
  const { size, isSolid } = getMarkerConfig(isRecommended, zoom)
...
export default function PropertyMap({
  ...
  const updateMarkers = useCallback(() => {
    ...
    properties.forEach(property => {
      ...
      const { size } = getMarkerConfig(isRecommended, currentZoom)
      
      const marker = L.marker(property.coordinates, {
        icon: createIcon(isRecommended, property.status, logoUrl, currentZoom),
      }).addTo(map)
  ...
  }, [properties, onPropertyClick, navigate])
